...
//! ported from `unidpp-registry`'s test support): no
//! reqwest/hyper-client/TLS dependency in either the service or its
//! tests. `http://` only; the gateway talks to the sibling issuer on
//! loopback or inside the same trust boundary, and production TLS
//! terminates at a fronting proxy.
//!
//! Consumers: the issuer upstream ([`crate::source::PassportSource`])
//! and the integration tests (driving this service itself).
...
impl Url {
    ...
    /// Percent-encode a path or query component (everything outside
...
/// Convenience wrapper for JSON requests (the issuer upstream and the
...
mod tests {
    ...
    #[test]
    fn url_parsing() {
        let u = Url::parse("http://127.0.0.1:8090/passports/x?at=1").unwrap();
        assert_eq!(u.host, "127.0.0.1");
        assert_eq!(u.port, 8090);
        assert_eq!(u.path_and_query, "/passports/x?at=1");
    ...
    #[test]
    fn component_encoding_keeps_urns_safe() {
        assert_eq!(
            Url::encode_query_component("urn:iso:std:iso-iec:15459:unidpp:passport:8"),
            "urn%3Aiso%3Astd%3Aiso-iec%3A15459%3Aunidpp%3Apassport%3A8"
        );
